...
public class KakaoAuthServiceTest {
	...
	public void kakaoJwtTest() throws IOException, NullPointerException {
			...
			Mono<Kakao.User> monoUser = Mono.just(
				new ObjectMapper().readValue(new ClassPathResource("/auth/kakaouser.json").getFile(),
					Kakao.User.class));
			...
			//then
			String userAccessToken = jwtTokenProvider.create(user.getId(), properties.getSecret());
